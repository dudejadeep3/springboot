...
public class GreetingController extends BaseController{
	...
	@RequestMapping(value="/api/greetings", method = RequestMethod.PUT, consumes = MediaType.APPLICATION_JSON_VALUE,produces=MediaType.APPLICATION_JSON_VALUE)
	...
	public ResponseEntity<Greeting> deleteGreeting(@RequestBody Greeting greeting, @PathVariable("id") Long id){
	...
	public ResponseEntity<Greeting> sendGreeting(@PathVariable("id") Long id,@RequestParam(value="wait",defaultValue="false") boolean waitForAsyncResult){
		...
		try {
			...
			if(waitForAsyncResult) {
				Future<Boolean> asyncResult = emailService.sendAsyncWithResult(greeting);
			...
			logger.error("A problem occured while sending the Greeting",e);
			return new ResponseEntity<Greeting>(HttpStatus.INTERNAL_SERVER_ERROR);
		}
		logger.info("<sendGreeting");
		return new ResponseEntity<Greeting>(greeting,HttpStatus.OK);
